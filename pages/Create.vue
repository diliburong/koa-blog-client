            ...
            <div class="new-category-add hidden-sm-and-up" @click="openList()" >
              ...
              <ul class="category-list">
                ...
                <transition name="slide-fade" mode="in-out">
                  ...
                  <!-- <li id="1" class="category-item" v-bind:class="{ 'hidden-xs-only': isCategoryClose }">Javascript</li> -->
                </transition>
              </ul>
...
export default {
  ...
  methods: {
    ...
    async submitArticle() {
      ...
      let result = await fetch.post(url, data);

      if(result.status === 200) {
        this.$router.push({ path: '/article/' + result.articleId }) 
      } else {
        Message({
    ...
    async createCategory() {
      ...
      let result = await fetch.post(url,  {newCategoryName} );
